...
void create_image_from_text(const char *filename1, const char *filename2, const char *filename3)
{
    ...
    cairo_surface_t *surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, 1000, 15000);
    ...
    // Draw text from third file
    while ((character = fgetc(file3)) != EOF)
    {
        cairo_move_to(cr, x, y);
...
void button_clicked(GtkButton *button, gpointer data)
{
    ...
    create_image_from_text("scheduler.pref", "memory.log", "scheduler.log");
    printf("\nHello from GUI\n");
